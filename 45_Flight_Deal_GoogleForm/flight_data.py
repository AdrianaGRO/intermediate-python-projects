...
class FlightData:
    ...
    def __init__(self, price, origin_airport, destination_airport, out_date, return_date, stops=0):
        ...
        - stops: Number of stops in the flight (0 for direct flights). Defaults to 0.
        """
        self.price = price
...
def find_cheapest_flight(data):
    ...
    # Count stops and get the final destination (not just the stopover)
    outbound_segments = first_flight["itineraries"][0]["segments"]
    outbound_stops = len(outbound_segments) - 1  # Number of stops = segments - 1
    destination = outbound_segments[-1]["arrival"]["iataCode"]  # Last segment's arrival is final destination
    
    out_date = first_flight["itineraries"][0]["segments"][0]["departure"]["at"].split("T")[0]
    return_date = first_flight["itineraries"][1]["segments"][0]["departure"]["at"].split("T")[0]

    # Initialize FlightData with the first flight for comparison
    cheapest_flight = FlightData(lowest_price, origin, destination, out_date, return_date, outbound_stops)
    ...
    for flight in data["data"]:
        ...
        if price < lowest_price:
            ...
            # Count stops and get the final destination for this flight
            ...
            cheapest_flight = FlightData(lowest_price, origin, destination, out_date, return_date, outbound_stops)
            print(f"Lowest price to {destination} is £{lowest_price} with {outbound_stops} stops")
