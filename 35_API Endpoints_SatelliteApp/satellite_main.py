...
# Check if it's night time in Bucharest
def is_night():
    MY_LAT = 44.4268
    MY_LONG = 26.1025
...
bucharest_long = 26.1025
bucharest_position = (bucharest_lat, bucharest_long)
print(f"Bucharest position: {bucharest_position}")

# Check ISS position and send notification if conditions are met
def check_iss_and_notify():
    ...
    if response.status_code == 200:
        ...
        # Check if ISS is close to Bucharest (within 5 degrees)
        if abs(iss_position[0] - bucharest_position[0]) <= 5 and abs(iss_position[1] - bucharest_position[1]) <= 5:
            print("The ISS is close to Bucharest!")
            ...
            print("The ISS is not close to Bucharest.")
            print(f"Distance: Lat {abs(iss_position[0] - bucharest_position[0]):.2f}°, Lng {abs(iss_position[1] - bucharest_position[1]):.2f}°")
            return False
    else:
